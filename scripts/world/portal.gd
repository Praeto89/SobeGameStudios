...
func _on_body_entered(body: Node2D) -> void:
	...
	if target_scene != "":
		...
		# ── Level-Wechsel ──────────────────────────────────
		# Vor dem Wechsel automatisch speichern: Fortschritt + Ziel-Szene, damit
		# "Fortsetzen" im Hauptmenue genau hier wieder aufsetzt.
		GameManager.save_game(target_scene)
